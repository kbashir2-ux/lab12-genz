...
from common.methods import anonymize, anonymizers, deanonymize, genz_anonymize
...
def test_given_anonymize_called_with_genz_then_expected_valid_response_returned():
    ...
    response_status, response_content = genz_anonymize(request_body)
    ...
    assert all("operator" in item and item["operator"] == "genz" for item in response_json["items"])
